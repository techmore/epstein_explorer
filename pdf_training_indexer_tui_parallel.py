...
def _ensure_nltk_data() -> None:
    """
    Ensure tokenizers exist in the current process.
    """
    try:
        nltk.data.find("tokenizers/punkt")
    except LookupError:
...
def _worker_init() -> None:
    """
    Initialize per-process NLP resources once.
    """
    global _WORKER_NLP
    _ensure_nltk_data()
...
def _process_file_payload(root_str: str, file_path_str: str) -> Dict[str, Any]:
    """
    Worker task: parse one file and return rows/counters for DB writer.
    """
    global _WORKER_NLP
    root = Path(root_str)
    ...
    for doc, sent in zip(_WORKER_NLP.pipe(sentences, batch_size=64), sentences):
        ...
        for ent in doc.ents:
            ...
            if len(ent_text) < 2:
                continue
            entity_rows.append((ent_text, mapped, top_folder, rel_path, file_name, sent[:MAX_SNIPPET_LEN]))
    ...
    pair_counter_serialized = {f"{a}\t{b}": int(cnt) for (a, b), cnt in pair_counter.items()}
    ...
    return {
        ...
        "pair_counter": pair_counter_serialized,
        "snippet_rows": snippet_rows,
    }


def _process_file_payload_file(root_str: str, file_path_str: str, payload_dir: str) -> Dict[str, Any]:
    """
    Worker task for parallel mode.

    Large per-file results are written to a temp pickle file so the process pool
    returns only a tiny descriptor instead of moving megabytes through IPC.
    """
    payload = _process_file_payload(root_str, file_path_str)
    if not payload.get("ok"):
        return payload

    fd, payload_path = tempfile.mkstemp(prefix="pdf_payload_", suffix=".pickle", dir=payload_dir)
    try:
...
def _load_payload_descriptor(descriptor: Dict[str, Any]) -> Dict[str, Any]:
    payload_path = descriptor.get("payload_path")
    if not payload_path:
        return descriptor
...
def _apply_file_payload(conn: sqlite3.Connection, payload: Dict[str, Any]) -> Tuple[int, int, int]:
    """
    Single-writer DB apply step for one worker payload.
    Returns: (word_rows, entity_rows, pair_rows)
    """
    top_folder = payload["top_folder"]
    rel_path = payload["rel_path"]
    file_name = payload["file_name"]

    word_counter = payload.get("word_counter", {})
    entity_rows = payload.get("entity_rows", [])
    pair_counter_serialized = payload.get("pair_counter", {})
    snippet_rows = payload.get("snippet_rows", [])

    pair_items: List[Tuple[str, str, int]] = []
    for key, cnt in pair_counter_serialized.items():
        if "\t" not in key:
            continue
        a, b = key.split("\t", 1)
        pair_items.append((a, b, int(cnt)))
    ...
    with conn:
        ...
        if pair_items:
            conn.executemany(
                ...
                DO UPDATE SET count=excluded.count
                """,
                [(a, b, cnt, top_folder, rel_path, file_name) for a, b, cnt in pair_items],
            )

            for a, b, cnt in pair_items:
                conn.execute(
                    """
    ...
    return (len(word_counter), len(entity_rows), len(pair_items))


def run_indexing(conn: sqlite3.Connection, root: Path, workers: int) -> None:
    """
    ...
    for file_path in files:
        try:
            if should_process_file(conn, file_path):
    ...
    if workers == 1:
        progress = tqdm(files_to_process, desc="Indexing PDFs", unit="file")
        for file_path in progress:
            try:
                payload = _process_file_payload(root_str, str(file_path))
                if not payload.get("ok"):
                    errors += 1
                    continue
                w, e, p = _apply_file_payload(conn, payload)
                processed += 1
    ...
    progress = tqdm(total=len(files_to_process), desc=f"Indexing PDFs ({workers} workers)", unit="file")
    max_in_flight = max(workers * 3, workers)
    file_iter = iter(files_to_process)

    with tempfile.TemporaryDirectory(prefix="pdf_index_payloads_") as payload_dir:
        with concurrent.futures.ProcessPoolExecutor(max_workers=workers, initializer=_worker_init) as pool:
            ...
            def submit_next() -> bool:
                try:
                    ...
                    return False
                fut = pool.submit(_process_file_payload_file, root_str, str(next_file), payload_dir)
                pending[fut] = str(next_file)
                return True
            ...
            while pending:
                ...
                for fut in done:
                    ...
                    try:
                        descriptor = fut.result()
                        payload = _load_payload_descriptor(descriptor)
                        if not payload.get("ok"):
                            errors += 1
                        else:
                            w, e, p = _apply_file_payload(conn, payload)
...
def run_tui(conn: sqlite3.Connection, root: Path, workers: int) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            run_indexing(conn, root, workers)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Number of worker processes for indexing (default: {DEFAULT_WORKERS}). Use 1 for single-process.",
    )
    ...
    try:
        if not args.skip_index:
            run_indexing(conn, root, args.workers)
        run_tui(conn, root, args.workers)
    except KeyboardInterrupt:
